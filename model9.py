...
# ============================================
# 模块 6. 定义三模态权重学习模型 (Softmax 权重 + 参数统计)
# ============================================
class MultiModalModel(nn.Module):
    def __init__(self, bert, roberta, vit, clip_visual,
        ...
        # ===== 模态权重计算器 =====
        self.weight_net = nn.Linear(hidden_dim, 1)
    ...
    def forward(self, bert_input_ids, bert_attention_mask,
        ...
        text_feat = (bert_feat + roberta_feat) / 2
        ...
        image_feat = (vit_feat + clip_feat) / 2

        # 社交特征
        social_feat = self.social_proj(social_features)

        # ===== 模态权重学习 =====
        feats = torch.stack([text_feat, image_feat, social_feat], dim=1)   # [B, 3, H]
        logits = self.weight_net(feats).squeeze(-1)                        # [B, 3]
        weights = torch.softmax(logits, dim=1)                             # [B, 3]

        # 加权融合
        fused = (weights.unsqueeze(-1) * feats).sum(dim=1)                 # [B, H]
