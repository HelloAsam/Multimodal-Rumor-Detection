...
class MultiModalModel(nn.Module):
    def __init__(self, bert, roberta, vit, clip_visual, social_dim=16, hidden_dim=256, num_classes=2, unfreeze_layers=2):
        ...
        # ===== Step2: 解冻每个模型的最后 unfreeze_layers 层 =====
        # BERT / RoBERTa
        ...
        self.clip_proj = nn.Linear(768, hidden_dim)   # CLIP 输出 768
    ...
    def forward(self, bert_input_ids, bert_attention_mask,
        ...
        bert_feat = self.bert_proj(bert_out.pooler_output)

        roberta_out = self.roberta(input_ids=roberta_input_ids, attention_mask=roberta_attention_mask)
        roberta_feat = self.roberta_proj(roberta_out.pooler_output)

        vit_out = self.vit(pixel_values=vit_pixel_values)
        vit_feat = self.vit_proj(vit_out.pooler_output)

        clip_out = self.clip_visual(pixel_values=clip_pixel_values)
        clip_feat = self.clip_proj(clip_out.pooler_output)
