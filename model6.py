...
# ============================================
# 模块 6. 定义四模态模型 (concat + Cross-Attention 融合, 解冻最后4层)
# ============================================
class MultiModalModel(nn.Module):
    def __init__(self, bert, roberta, vit, clip_visual, social_dim=16, hidden_dim=256, num_classes=2, unfreeze_layers=4):
        super().__init__()
        self.bert = bert
        ...
        # ===== Cross-Attention =====
        # 文本 concat 后 512, 图像 concat 后 512
        self.cross_attn_text = nn.MultiheadAttention(embed_dim=512, num_heads=8, batch_first=True)
        self.cross_attn_image = nn.MultiheadAttention(embed_dim=512, num_heads=8, batch_first=True)

        # ===== 分类器 =====
        # 输入维度: 512 (text+image cross-attn) + 256 (social) = 768
        ...
        print("📊 模型参数量统计 (解冻最后两层 + Cross-Attention):")
    ...
    def forward(self, bert_input_ids, bert_attention_mask,
        ...
        roberta_out = self.roberta(input_ids=roberta_input_ids, attention_mask=roberta_attention_mask)
        bert_feat = self.bert_proj(bert_out.pooler_output)       # [B, 256]
        roberta_feat = self.roberta_proj(roberta_out.pooler_output) # [B, 256]
        text_feat = torch.cat([bert_feat, roberta_feat], dim=1)      # [B, 512]

        # ===== 图像模态 =====
        vit_out = self.vit(pixel_values=vit_pixel_values)
        clip_out = self.clip_visual(pixel_values=clip_pixel_values)
        vit_feat = self.vit_proj(vit_out.pooler_output)           # [B, 256]
        clip_feat = self.clip_proj(clip_out.pooler_output)        # [B, 256]
        image_feat = torch.cat([vit_feat, clip_feat], dim=1)      # [B, 512]

        # ===== Cross-Attention =====
        text_feat_seq = text_feat.unsqueeze(1)   # [B,1,512]
        image_feat_seq = image_feat.unsqueeze(1) # [B,1,512]

        # Text as Query, Image as KV
        text2img, _ = self.cross_attn_text(text_feat_seq, image_feat_seq, image_feat_seq)
        # Image as Query, Text as KV
        img2text, _ = self.cross_attn_image(image_feat_seq, text_feat_seq, text_feat_seq)

        multimodal_feat = (text2img.squeeze(1) + img2text.squeeze(1)) / 2  # [B,512]

        # ===== 社交特征 =====
        social_feat = self.social_proj(social_features)    # [B,256]

        # ===== 融合 + 分类 =====
        fused = torch.cat([multimodal_feat, social_feat], dim=1)   # [B,768]
