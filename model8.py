...
# ============================================
# 模块 6. 定义四模态模型 (门控 + 模态 Dropout + 参数统计)
# ============================================
class MultiModalModel(nn.Module):
    def __init__(self, bert, roberta, vit, clip_visual,
                 social_dim=16, hidden_dim=256, num_classes=2,
                 unfreeze_layers=2, modality_dropout=0.2):
        super().__init__()
        self.bert = bert
        ...
        self.roberta_proj = nn.Linear(768, hidden_dim)
        self.vit_proj = nn.Linear(768, hidden_dim)
        self.clip_proj = nn.Linear(768, hidden_dim)
        self.social_proj = nn.Linear(social_dim, hidden_dim)

        # ===== 门控层 (为每个模态学习一个 gate) =====
        self.bert_gate = nn.Sequential(nn.Linear(hidden_dim, 1), nn.Sigmoid())
        self.roberta_gate = nn.Sequential(nn.Linear(hidden_dim, 1), nn.Sigmoid())
        self.vit_gate = nn.Sequential(nn.Linear(hidden_dim, 1), nn.Sigmoid())
        self.clip_gate = nn.Sequential(nn.Linear(hidden_dim, 1), nn.Sigmoid())
        self.social_gate = nn.Sequential(nn.Linear(hidden_dim, 1), nn.Sigmoid())
        ...
        def format_all(n):
            return f"{n}  |  {n/1e3:.2f}K  |  {n/1e6:.2f}M"

        print("📊 模型参数量统计 (解冻最后 {} 层):".format(unfreeze_layers))
        print(f"  总参数: {format_all(total_params)}")
        print(f"  可训练参数: {format_all(trainable_params)}")
        print(f"  冻结参数: {format_all(frozen_params)}")
    ...
    def forward(self, bert_input_ids, bert_attention_mask,
        ...
        # 文本
        bert_feat = self.bert_proj(self.bert(input_ids=bert_input_ids,
                                             attention_mask=bert_attention_mask).pooler_output)
        roberta_feat = self.roberta_proj(self.roberta(input_ids=roberta_input_ids,
                                                      attention_mask=roberta_attention_mask).pooler_output)

        # 图像
        vit_feat = self.vit_proj(self.vit(pixel_values=vit_pixel_values).pooler_output)
        clip_feat = self.clip_proj(self.clip_visual(pixel_values=clip_pixel_values).pooler_output)

        # 社交特征
        social_feat = self.social_proj(social_features)

        # ===== 门控加权 =====
        bert_feat = self.bert_gate(bert_feat) * bert_feat
        roberta_feat = self.roberta_gate(roberta_feat) * roberta_feat
        vit_feat = self.vit_gate(vit_feat) * vit_feat
        clip_feat = self.clip_gate(clip_feat) * clip_feat
        social_feat = self.social_gate(social_feat) * social_feat

        # ===== 模态 Dropout (训练时随机丢弃一些模态) =====
        if self.training and self.modality_dropout > 0:
            if torch.rand(1).item() < self.modality_dropout:
                bert_feat = torch.zeros_like(bert_feat)
            if torch.rand(1).item() < self.modality_dropout:
                roberta_feat = torch.zeros_like(roberta_feat)
            if torch.rand(1).item() < self.modality_dropout:
                vit_feat = torch.zeros_like(vit_feat)
            if torch.rand(1).item() < self.modality_dropout:
                clip_feat = torch.zeros_like(clip_feat)
            if torch.rand(1).item() < self.modality_dropout:
                social_feat = torch.zeros_like(social_feat)

        # 融合
        fused = torch.cat([bert_feat, roberta_feat, vit_feat, clip_feat, social_feat], dim=1)
